...
app.post(["/api/analyze-xml", "/analyze-xml"], (req, res) => {
  try {
    ...
    const analyzed = xmlFiles.map((file: any) => {
      ...
      // 1. Trích xuất thông tin từ thẻ <TTKhac> (BẮT BUỘC theo yêu cầu)
      // Tìm khối <TTKhac> ... </TTKhac>
      const ttKhacRegex = /<TTKhac[\s\S]*?>([\s\S]*?)<\/TTKhac>/i;
      const ttKhacMatch = content.match(ttKhacRegex);

      if (ttKhacMatch) {
        const ttKhacContent = ttKhacMatch[1];
        
        // Tìm mã tra cứu trong <TTKhac>
        // Các thẻ phổ biến: <MTC>, <MaTraCuu>, <MaTraCuuHDon>, <MTCHon>, <MaTraCuuHD>, <Value> gắn với MaTraCuu
        const codeRegexes = [
          /<MTC[\s\S]*?>([^<]+)<\/MTC>/i,
          ...
          /<Key[\s\S]*?>MTC<\/Key>\s*<Value[\s\S]*?>([^<]+)<\/Value>/i
        ];

        for (const regex of codeRegexes) {
          const match = ttKhacContent.match(regex);
          if (match && match[1]) {
            code = match[1].trim();
            break;
          }
        ...
        // Tìm link tra cứu trong <TTKhac>
        // Các thẻ phổ biến: <LinkTraCuu>, <TrangWebTraCuu>, <URLTraCuu>, <TrangWeb>, <Link>, <Value> chứa http
        const webRegexes = [
      ...
      // Nếu không tìm thấy mã tra cứu trong khối <TTKhac>
      if (!code) {
        status = "invalid";
        errorDescription = "Không tìm thấy mã tra cứu trong thẻ <TTKhac>.";
      }
